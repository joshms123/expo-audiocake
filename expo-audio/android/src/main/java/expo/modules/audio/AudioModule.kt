...
class AudioModule : Module() {
  ...
  // Preserved: recording-specific audio focus configuration
  // When allowsBackgroundRecording is set, use exclusive focus gain + speech content type
  // to prevent audio interruptions during recording
  ...
  private fun requestAudioFocus() {
    ...
    val result = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      // Preserved: use exclusive focus and speech content type for recording mode
      val requestType = when {
        isRecordingMode -> AudioManager.AUDIOFOCUS_GAIN_TRANSIENT_EXCLUSIVE
        interruptionMode == InterruptionMode.DO_NOT_MIX -> AudioManager.AUDIOFOCUS_GAIN_TRANSIENT
        else -> AudioManager.AUDIOFOCUS_GAIN_TRANSIENT_MAY_DUCK
      }
      val contentType = if (isRecordingMode) AudioAttributes.CONTENT_TYPE_SPEECH else AudioAttributes.CONTENT_TYPE_MUSIC
      val usage = if (isRecordingMode) AudioAttributes.USAGE_VOICE_COMMUNICATION else AudioAttributes.USAGE_MEDIA
  ...
  override fun definition() = ModuleDefinition {
    ...
    Class(AudioRecorder::class) {
      ...
      // Preserved: force reset audio session to clear VoIP app state (e.g., LiveKit)
