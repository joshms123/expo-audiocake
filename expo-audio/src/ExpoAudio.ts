 ...
 * 
 * This function returns information about the current AVAudioSession configuration
 * including category, mode, sample rate, buffer duration, and output route.
 * Returns `null` on Android and Web platforms.
 *
 * @platform ios
 * @returns Current audio session state or `null` if not available on this platform.
 *
 * @example
 * ```tsx
 * import { getAudioSessionState } from 'expo-audio';
 *
 * const debugAudioSession = () => {
 *   const state = getAudioSessionState();
 *   if (state) {
 *     console.log('Category:', state.category);
 *     console.log('Mode:', state.mode);
 *     console.log('Sample Rate:', state.sampleRate, 'Hz');
 *     console.log('Buffer Duration:', state.ioBufferDuration, 's');
 *     console.log('Output Route:', state.outputRoute);
 *   } else {
 *     console.log('Audio session state not available on this platform');
 *   }
 * };
 * ```
 */
